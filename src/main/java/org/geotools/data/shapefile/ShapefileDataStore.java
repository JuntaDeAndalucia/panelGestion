...
public class ShapefileDataStore extends AbstractFileDataStore {
    ...
    */
   private static final Set HINTS = Collections.unmodifiableSet(new HashSet(Arrays
      .asList(new Object[] { Hints.FEATURE_DETACHED, Hints.SCREENMAP })));
   ...
   protected boolean useMemoryMappedBuffer = false; // windows is not up to use memory mapping in
                                                    // anger
   ...
   /**
    * this sets the datastore's namespace during construction (so the schema - FeatureType - will
    * have the correct value) You can call this with namespace = null, but I suggest you give it an
    * actual namespace.
    * 
    * @param url
    * @param namespace
    ...
    * @patch
    * 
    * It uses the user name and the password to build the shapefile
    * 
    * @param url
    * @param user
    * @param password
    * @param namespace
    * @param useMemoryMapped
    * @param cacheMemoryMap
    * @param dbfCharset
    * @throws java.net.MalformedURLException
    */
   public ShapefileDataStore(URL url, String user, String password, URI namespace,
      boolean useMemoryMapped, boolean cacheMemoryMap, Charset dbfCharset)
      throws java.net.MalformedURLException {
      this.namespace = namespace;
      shpFiles = new ShpFiles(url, user, password);
      if (!isLocal() || !shpFiles.exists(SHP)) {
         this.useMemoryMappedBuffer = false;
      }
      else {
   ...
   /**
    * Used by SimpleFeatureSource / FeatureStore / FeatureLocking to access a single ResourceInfo.
    * 
    * @param typeName
    * @return ResourceInfo
    */
   ...
   /**
    * Just like the basic version, but adds a small optimization: if no attributes are going to be
    * read, don't uselessly open and read the dbf file. Makes sure to consider also attributes in
    * the query.
    * 
   ...
   protected FeatureReader<SimpleFeatureType, SimpleFeature> getFeatureReader(String typeName,
      ...
      if ((propertyNames != null)
         ...
         try {
            ...
            return createFeatureReader(typeName, getAttributesReader(false, query, propertyNames),
               newSchema);
         }
         catch (SchemaException se) {
            throw new DataSourceException("Error creating schema", se);
      ...
      try {
         ...
         return createFeatureReader(typeName, getAttributesReader(true, query, requiredProperties),
            newSchema);
      }
      catch (SchemaException se) {
         throw new DataSourceException("Error creating schema", se);
   ...
   protected org.geotools.data.FIDFeatureReader createFeatureReader(String typeName,
      ShapefileAttributeReader reader, SimpleFeatureType readerSchema) throws SchemaException {
   ...
   /**
    * Returns the attribute reader, allowing for a pure shapefile reader, or a combined dbf/shp
    * reader.
    * 
    * @param readDbf
   ...
   protected DbaseFileReader openDbfReader() throws IOException {
      ...
      try {
         return new DbaseFileReader(shpFiles, useMemoryMappedBuffer, dbfCharset, dbfTimeZone);
      }
      catch (IOException e) {
   ...
   /**
    * Get an array of type names this DataStore holds.<BR/>
    ...
    * For example, if the urls path is file:///home/billy/mytheme.shp, the type name will be
    * mytheme.
    * 
   ...
   protected List<AttributeDescriptor> readAttributes() throws IOException {
      ...
      try {
         Class<?> geometryClass = JTSUtilities
            .findBestGeometryClass(shp.getHeader().getShapeType());
   ...
   /**
    * Set the FeatureType of this DataStore. This method will delete any existing local resources or
    * throw an IOException if the DataStore is remote.
    * 
    * @param featureType
   ...
   /**
    * Gets the bounding box of the file represented by this data store as a whole (that is, off all
    * of the features in the shapefile)
    * 
    * @return The bounding box of the datasource or null if unknown and too expensive for the method
    *         to calculate.
    * 
   ...
   protected void finalize() throws Throwable {
      ...
      if (shpFiles != null && trace != null) {
         LOGGER.log(Level.SEVERE,
            "Undisposed of shapefile, you should call dispose() on all shapefile stores", trace);
      }
      dispose();
   }
