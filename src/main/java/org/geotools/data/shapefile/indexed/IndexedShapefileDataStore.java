...
public class IndexedShapefileDataStore extends ShapefileDataStore implements FileWriter {
   ...
   public IndexedShapefileDataStore(URL url, URI namespace, boolean useMemoryMappedBuffer,
      boolean createIndex, IndexType treeType) throws MalformedURLException {
   ...
   public IndexedShapefileDataStore(URL url, URI namespace, boolean useMemoryMappedBuffer,
      boolean cacheMemoryMaps, boolean createIndex, IndexType treeType, Charset dbfCharset)
      throws java.net.MalformedURLException {
   ...
   // TODO
   public IndexedShapefileDataStore(URL url, String user, String password, URI namespace,
      boolean useMemoryMappedBuffer, Boolean cacheMemoryMaps, boolean createIndex, IndexType treeType,
      Charset dbfCharset) throws MalformedURLException {
      super(url, user, password, namespace, useMemoryMappedBuffer, cacheMemoryMaps, dbfCharset);
    ...
    * 
    * @param force
    *           Forces the index re-creation even if the spatial index seems to be up to date
   ...
   public boolean createSpatialIndex(boolean force) {
      ...
      try {
         if (shpFiles.isLocal() && createIndex && (needsGeneration(treeType.shpFileType) || force)) {
            createSpatialIndex();
            return true;
         }
      }
   ...
   public FeatureWriter<SimpleFeatureType, SimpleFeature> getFeatureWriterAppend(String typeName,
      ...
      if (transaction == null) {
         throw new NullPointerException("getFeatureWriter requires Transaction: "
   ...
   /**
    * Use the spatial index if available and adds a small optimization: if no attributes are going
    * to be read, don't uselessly open and read the dbf file.
    * 
   ...
   protected FeatureReader<SimpleFeatureType, SimpleFeature> getFeatureReader(String typeName,
      ...
      String[] propertyNames = query.getPropertyNames() == null ? new String[0] : query
         .getPropertyNames();
      ...
      try {
         if (((query.getPropertyNames() != Query.NO_NAMES) && (propertyNames.length == 1) && propertyNames[0]
         ...
         return createFeatureReader(typeName,
            getAttributesReader(readDbf, readGeometry, query, newSchema), newSchema);
      }
   ...
   /**
    * Much like {@link DataUtilities#createSubType(SimpleFeatureType, String[])}, but makes sure to
   ...
   protected FeatureReader<SimpleFeatureType, SimpleFeature> createFeatureReader(String typeName,
      IndexedShapefileAttributeReader r, SimpleFeatureType readerSchema) throws SchemaException,
      IOException {
      ...
      return new org.geotools.data.FIDFeatureReader(r, fidReader, readerSchema);
   }
    ...
    * Will return null if reading the indexes confirms there is nothing to read
    * 
    * @param readDbf
   ...
   protected IndexedShapefileAttributeReader getAttributesReader(boolean readDbf,
      boolean readGeometry, Query query, SimpleFeatureType targetSchema) throws IOException {
      ...
      if (goodRecs != null && !goodRecs.hasNext()) {
         // System.out.println("Empty results for " + targetSchema.getName().getLocalPart() +
         // ", skipping read");
         goodRecs.close();
         return null;
      }
    ...
    * 
    * @param fids
    *           the fids of the features to find. If the set is sorted by alphabet the performance
   ...
   /**
    * Returns true if the indices already exist and do not need to be regenerated or cannot be
    * generated (IE isn't local).
    * 
    ...
    *            DOCUMENT ME!
    */
   protected CloseableIterator<Data> queryQuadTree(Envelope bbox) throws DataSourceException,
   ...
   protected FeatureWriter<SimpleFeatureType, SimpleFeature> createFeatureWriter(String typeName,
      ...
      return new IndexedShapefileFeatureWriter(typeName, shpFiles, attReader, featureReader, this,
   ...
   protected ReferencedEnvelope getBounds(Query query) throws IOException {
      ...
      Set<Identifier> fids = (Set<Identifier>) filter.accept(
         IdCollectorFilterVisitor.IDENTIFIER_COLLECTOR, new TreeSet<Identifier>(
    ...
    * 
    *      if (isWriteable) { if (getLockingManager() != null) { return new AbstractFeatureLocking()
    *      { public DataStore getDataStore() { return IndexedShapefileDataStore.this; }
    * 
    ...
    * 
    *      public SimpleFeatureType getSchema() { return featureType; }
    * 
    *      public ReferencedEnvelope getBounds(Query query) throws IOException { return
    *      IndexedShapefileDataStore.this.getBounds(query); } }; } else { return new
    *      AbstractFeatureStore() { public DataStore getDataStore() { return
    *      IndexedShapefileDataStore.this; }
    * 
    *      public void addFeatureListener(FeatureListener listener) {
    *      listenerManager.addFeatureListener(this, listener); }
    * 
    ...
    *      public ReferencedEnvelope getBounds(Query query) throws IOException { return
    *      IndexedShapefileDataStore.this.getBounds(query); } }; } } else { return new
    *      AbstractFeatureSource() { public DataStore getDataStore() { return
    *      IndexedShapefileDataStore.this; }
    * 
    *      public void addFeatureListener(FeatureListener listener) {
    *      listenerManager.addFeatureListener(this, listener); }
    ...
    * 
    *      public ReferencedEnvelope getBounds(Query query) throws IOException { return
    *      IndexedShapefileDataStore.this.getBounds(query); } }; } }
    */
   ...
   /**
    * Builds the QuadTree index. Usually not necessary since reading features will index when
    * required
    * 
    * @throws TreeException
    */
